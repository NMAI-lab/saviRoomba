...
def rosMain():
    ...
    while not rospy.is_shutdown():
        timer = time.time() # take time at start of loop
        # Initialize values
        samples = dict()
        sumDist = dict()
        prevdist = dict()
        ...
        if valid:
            ...
            if not to_send == '':
                timer = time.time() - timer # record the time between start of loop and publish perception
                rospy.loginfo("Time taken to publish perception: " + str(timer))
